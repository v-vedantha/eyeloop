...
class custom_Extractor:
    def __init__(self, screen_w=1000, screen_h=1000, camera_w=1000, camera_h=1000, buffer=None):
    ...
    def get_input(self):
        if self.buffer is not None:
            return coordinate_mapping.Input(buffer=self.buffer)
        if self.most_recent_pupil is None:
            return None
        frame = self.camera.read()[1]
        world_points = self.world_detector(frame)
        if world_points is None:
            return None
        return coordinate_mapping.Input(self.most_recent_pupil, world_points[0], world_points[1])
